...
GPtrArray*bicmag_ntis_parse_attachments(const gchar*html, const gchar*base_uri, GError**error)
{
    ...
    g_autoptr(GRegex) re =
        g_regex_new(
            "<a[^>]*href=\\\"([^\\\"]+)\\\"[^>]*onclick=\\\"([^\\\"]*fn_fileDownload[^\\\"]*)\\\"[^>]*>(.*?)</a>",
    ...
    while (g_match_info_matches(mi)) {
        ...
        if (g_regex_match(args, onclick, 0, &ai)) {
            g_autoptr(BicMagAttachment) a = bicmag_attachment_new();
            a->id = g_match_info_fetch(ai,1); a->notice_id = g_match_info_fetch(ai,2);
            g_strstrip(name); a->name = g_strdup(name);
            g_autoptr(GUri) base = g_uri_parse(base_uri, 0, NULL);
            g_autoptr(GUri) uri = base ? g_uri_parse_relative(base, href, 0, NULL) : NULL;
            if (uri){a->download_url = g_uri_to_string(uri);
            } if (a->name && a->download_url){ g_ptr_array_add(out, g_steal_pointer(&a));}
        }
